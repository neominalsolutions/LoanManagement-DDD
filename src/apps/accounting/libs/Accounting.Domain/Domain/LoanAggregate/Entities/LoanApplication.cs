...
using Accounting.Domain.Domain.LoanAggregate.Enumerations;
using Accounting.Domain.Domain.LoanAggregate.Exceptions;
using Accounting.Domain.Domain.LoanAggregate.Services;
...
namespace Accounting.Domain.Domain.LoanAggregate.Entities
{
  // Kredi başvurusu
  public class LoanApplication:AggregateRoot
  {
    ...
    public string CustomerId { get; private set; } // Kredi başvurusu yapan müşteri
    ...
    public void CheckApproval(CreditScoreService creditScoreService)
    {
      ...
      else
      {
        ...
        var @event = new LoanApproved(Id, CustomerId, LoanAmount, Term, BankRate);
